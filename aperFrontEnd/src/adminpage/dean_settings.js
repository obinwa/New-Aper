...
class DeanLecturer extends Component  {
  ...
  getAllLecturer=(e)=>{   
    ...
    axios.get(`${URL}/hod/getAllLecturer/${e}`,options)
    .then( (response) => {
        if(response.data){         
          console.log(response.data)
  ...
  getDeanDepartments=()=>{   
    console.log("yea")   
    const options={headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.state.token}`}}; console.log(options);            
    axios.get(`${URL}/dean/getAllDepartmentForDean`,options)
    .then( (response) => {
        if(response.data){         
          console.log(response.data)
            this.setState({departments : response.data });
            // this.setState({department_id : response.data[0].id})
        }
    })
    .catch((error)=> {
        if(error.response){
                ...
                <div className="box-body">
                  ...
                  <div className="col-md-5">
                    <div className="form-group">
                      ...
                      <select onChange={event =>this.getAllLecturer(event.target.value)}
